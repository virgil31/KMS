Ext.define('CL.view.event_thread.V_list_by_event', {
    extend: 'Ext.panel.Panel',
    xtype: 'event_thread_list_by_event',
    itemId: 'event_thread_list_by_event_id',
    alias: 'widget.event_thread_list_by_event',
    ...
    listeners: {
        activate: function(){
            setTimeout(function(){
                Ext.StoreManager.lookup("S_event_thread").load({
                    params:{
            ...
            CL.app.getController("C_event").redirectTo("event/"+event_id+"/threads");
        }
    },
    ...
    items: [
        {
            ...
            tbar: [
                ...
                '->',
                {
                    xtype: 'label',
                    margin: '0 10 0 10',
                    html: '<a onclick="CL.app.getController(\'C_event_thread\').showInfo(this);return false;" href="#" style="color: #963232 !important; font-weight: bold; font-size: large;" ><u>Cosa sono le Discussioni e chi può aprirle/rispondere?</u></a>'
                }
            ],
            columns: [
                {
                ...
                },
                {
                    ...
                    renderer: function(value,metaData,record){
                        ...
                        if(prefix==null){
                            to_return = '<div style="font-weight: bold"><a  style="color:#404040 !important;" href="#event_thread/'+thread_id+'"><u>'+title+'</u></a></div>' +
                            ...
                            to_return = '<div style="color:blue; font-weight: bold;">'+prefix+'</div>' +
                                '<div style="font-weight: bold"><a  style="color:#404040 !important;" href="#event_thread/'+thread_id+'"><u>'+title+'</u></a></div>' +
                                '<i>Iniziata da <a style="color: #c70000 !important;" href="#user/'+created_by+'">'+created_by_name+' (#'+created_by+')</a>, '+created_at+'</i>';
                        }
                    ...
                    items: [

                        {
                            ...
                            handler: function(grid, rowIndex) {
                                ...
                                CL.app.getController("C_permessi").canWriteEventThread(event_id, true,function(){
                                            ...
                                            if (btn === 'yes'){
                                                Ext.Msg.prompt("Chiusura Discussione","Giustifica il motivo della chiusura:", function(btn, text){
                                                    if (btn == 'ok'){
                                                        ...
                                                        Ext.StoreManager.lookup("S_event_thread").sync();
                                                        setTimeout(function(){
                                                            Ext.toast({
                                                                title: 'Successo',
                            ...
                            handler: function(grid, rowIndex) {
                                ...
                                CL.app.getController("C_permessi").canWriteEventThread(event_id, true,function(){
                                    ...
                                    Ext.Msg.confirm('Attenzione!', 'Eliminare definitivamente la discussione <b>"'+rec.get("title")+"\"</b>?",function(btn){
                                        if (btn === 'yes'){
                                            Ext.StoreManager.lookup("S_event_thread").remove(rec);
                                            Ext.StoreManager.lookup("S_event_thread").sync();
