Ext.define('CL.controller.C_event_thread', {
    ...
    views: [
        'event_thread.V_list_by_event',
        'event_thread.V_create'
    ],
    ...
    init: function () {
        this.control({
            ...
            //ON CREATE
            'event_thread_list_by_event button[action=on_create]':{
                click: this.onCreate
            },

            //DO CREATE
            "event_thread_create button[action=do_create]":{
                click: this.doCreate
            }
    ...
    onCreate: function (btn) {
        if(Ext.util.Cookies.get("user_id") === null)
            ...
            Ext.widget("event_thread_create",{
        ...
        if(message_without_tags.length<50) {
            ...
            if(form.isValid()){
                if(Ext.util.Cookies.get("user_id")===null)
                    ...
                    Ext.StoreManager.lookup("S_event_thread").add(values);

                    Ext.ComponentQuery.query("event_thread_create")[0].mask("Attendere...");
                    Ext.StoreManager.lookup("S_event_thread").sync({
                        callback: function () {
                            Ext.ComponentQuery.query("event_thread_create")[0].unmask();

                            Ext.StoreManager.lookup("S_event_thread").reload();
    ...
    share: function(btn,rec_thread){
        var thread_id = rec_thread.get("id"),
            ...
            event_title = rec_thread.get("event_name");
                ...
                },
                {
                    ...
                    items:[
                        ...
                        },
                        {
                            ...
                            "background-color: transparent",
                            handler: function () {
                                window.open("https://twitter.com/share?url="+escape(window.location.href)+"&text=SITAR - Guarda la discussione '"+thread_prefix+" "+thread_title+"' sull'evento '"+event_title+"'!", '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');
                            }
                        }
                    ]
                }
