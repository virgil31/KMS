...
createUserActivity($pdo,$_COOKIE["user_id"],'chiuso la discussione <b>'.$data["prefix"].' '.$data["title"].'</b> nell\'evento <b>'.getEventTitle($pdo,$data["event_id"]).'</b>','event_thread/'.$data['id'],"icon_thread.png",$data["event_id"],null);
...
function inviaMessaggioChiusura($pdo,$thread_id,$closed_by,$close_message){
    ...
    $params = array(
        "thread_id" => $thread_id,
        "sent_by" => $closed_by,
        "message" => '<div style="font-weight: bold !important; color: green !important;">[DISCUSSIONE CHIUSA] Motivo:</div>'.$close_message
    );
    $statement->execute($params);
