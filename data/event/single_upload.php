...
//echo "Event_id alla quale associare i file: ".$_GET["event_id"];
...
if ($success) {
    //aggiungo in kms_event_file
    //aggiungo in st_file
    $s = $pdo->prepare("
        INSERT INTO kms_event_file(event_id,file_id,title,uploaded_by,uploaded_at)
        VALUES(:event_id,:file_id,:title,:uploaded_by,NOW())
    ");
    ...
    if ($success) {
        ...
        createUserActivity($pdo,$_COOKIE["user_id"],'caricato il documento <b>'.getEventFileTitle($pdo,$event_id,$id_to_assign).'</b> nell\'evento <b>'.getEventTitle($pdo,$event_id).'</b>','event/'.$event_id."/file/".$id_to_assign,"icon_file.png",$data["event_id"],null);
